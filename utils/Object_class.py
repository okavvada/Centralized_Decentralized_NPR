...
from utils.functions import find_nearest
...
class Pumps():
	...
	def headloss(self):
		...
		if headloss>0.1*self.elevation_head:
			headloss=0.1*self.elevation_head
		return headloss #m
...
class Treatment():
	...
	def MBR_operation(self):
		if self.status == 'current':
			...
			MBR_Operation_energy_KWh_m3=(9.5*(self.total_demand_m3_d)**(-0.3))*0.75*.8
			if 9.5*(self.total_demand_m3_d)**(-0.3)>7:
